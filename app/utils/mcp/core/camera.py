...
def get_onvif_rtsp_url(ip, port=80, user=None, password=None):
    if user is None:
        user = os.environ.get('CAMERA_USER', 'admin')
    if password is None:
        password = os.environ.get('CAMERA_PASSWORD', '123456')
    try:
...
def capture(question: str):
    ...
    if rtsp_url == '':
        ...
        camera_password = os.environ.get('CAMERA_PASSWORD', 'Cc5201314')
        rtsp_url = get_onvif_rtsp_url('192.168.10.145', 80, camera_user, camera_password)
    ...
    device_id = os.environ.get('DEVICE_ID', '5c:aa:f4:cc:b8:dd')
    client_id = os.environ.get('CLIENT_ID', '5c:aa:f4:cc:b8:dd')
    headers = {
